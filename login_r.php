...
<head>
	...
	//Checking is user existing in the database or not
        $query = "SELECT * FROM `doctor` WHERE email='$email' AND password=PASSWORD('$password')";
		$result = mysqli_query($conn,$query) or die(mysql_error());
		$rows = mysqli_num_rows($result);
        if($rows==1){
			$_SESSION['email'] = $email;
			echo "<script>location.href='doctor-dashboard.php'</script>"; // Redirect user to index.php
			}
			else{
